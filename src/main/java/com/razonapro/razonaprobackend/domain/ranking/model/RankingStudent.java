...
@Entity
@Table(name = "rankings_students")
public class RankingStudent {

    @Id
    @GeneratedValue(strategy = GenerationType.SEQUENCE, generator = "seq_ranking_student")
    @SequenceGenerator(name = "seq_ranking_student",
            sequenceName = "seq_ranking_student_id",
            allocationSize = 1)
    ...
    @Builder.Default
    private BigDecimal triedsScore = BigDecimal.ZERO;

    @Column(name = "ai_trieds_score", precision = 7, scale = 2, nullable = false)
    @Builder.Default
    private BigDecimal aiTriedsScore = BigDecimal.ZERO;

    @Column(name = "trieds_count", nullable = false)
    @Builder.Default
    private Integer triedsCount = 0;

    @Column(name = "ai_trieds_count", nullable = false)
    @Builder.Default
    private Integer aiTriedsCount = 0;
